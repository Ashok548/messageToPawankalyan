...
function SubmitLeaderContent() {
    ...
    const tAreas = useTranslations('leaders.serviceAreas');
    const tValues = useTranslations('leaders.values');
    const tValidation = useTranslations('leaders.validation');
    ...
    const validate = () => {
        ...
        if (formData.serviceAreas.length === 0) {
            newErrors.serviceAreas = tValidation('serviceAreaCount');
        } else if (formData.serviceAreas.length > 5) {
            newErrors.serviceAreas = tValidation('serviceAreaCount');
        }

        if (formData.values.length === 0) {
            newErrors.values = tValidation('valuesCount');
                ...
                <Paper sx={{ p: { xs: 3, sm: 4 }, boxShadow: '0 2px 8px rgba(0, 0, 0, 0.1)' }}>
                    <Typography variant="h5" sx={{ mb: 3, fontWeight: 600 }}>
                        ...
                        <Box component="form" onSubmit={handleSubmit} noValidate sx={{ display: 'flex', flexDirection: 'column', gap: 2.5 }}>
                            ...
                            <TextField
                                label={t('whySocietyNeeds')}
                                value={formData.reason}
                                onChange={(e) => handleChange('reason', e.target.value)}
                                error={!!errors.reason}
                                helperText={errors.reason || `${t('whySocietyNeedsHelper')} ${formData.reason.length}/2000`}
                                fullWidth
                                required
                                multiline
                                rows={6}
                            ...
                            <FormControl error={!!errors.serviceAreas}>
                                <FormLabel>{t('areaOfService')}</FormLabel>
                                <FormGroup>
                            ...
                            <FormControl error={!!errors.values}>
                                <FormLabel>{t('valuesRepresented')}</FormLabel>
                                <FormGroup>
                                    {VALUES.map((value) => (
                                        <FormControlLabel
                                            key={value}
