            ...
            <Container maxWidth={false} sx={{ px: { xs: 2, sm: 3 } }}>
                    ...
                    {isAdmin && (
                        <Box sx={{ display: 'flex', gap: 1 }}>
                            ...
                            {caseData.status !== 'ACTION_TAKEN' && caseData.status !== 'CLOSED' && (
                                <Tooltip title={t('recordDecision')}>
                                    <IconButton
                                        color="error"
                ...
                <Paper elevation={0} sx={{ p: { xs: 3, md: 5 }, borderRadius: 3, border: '1px solid', borderColor: 'divider' }}>
                    ...
                    {caseData.imageUrls && caseData.imageUrls.length > 0 && (
                        <Box sx={{ mt: 6 }}>
                            <Typography variant="subtitle2" sx={{ fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.05em', color: 'text.secondary', mb: 2 }}>
                                {t('evidenceGallery')} ({caseData.imageUrls.length})
                            </Typography>
                    ...
                    <Box sx={{ mb: 5 }}>
                        ...
                        <Grid container spacing={2}>
                            ...
                            <Grid item xs={12} sm={6} md={8} sx={{ display: 'flex', alignItems: 'center', gap: 3 }}>
                                <Typography variant="body2" color="text.secondary">
                                    <strong>{t('created')}:</strong> {new Date(caseData.createdAt).toLocaleDateString()}
                                </Typography>
                                {caseData.updatedAt && (
                                    <Typography variant="body2" color="text.secondary">
                                        <strong>{t('updated')}:</strong> {new Date(caseData.updatedAt).toLocaleDateString()}
                                    </Typography>
                                )}
                            </Grid>
                        </Grid>
                ...
                <Dialog open={statusModalOpen} onClose={() => setStatusModalOpen(false)}>
                    ...
                    <DialogActions>
                        <Button onClick={() => setStatusModalOpen(false)}>{tCommon('cancel')}</Button>
                        <Button onClick={handleStatusUpdate} variant="contained" disabled={!newStatus}>{tCommon('update') || 'Update'}</Button>
                    </DialogActions>
                </Dialog>

                {/* Record Decision Dialog */}
                <Dialog open={decisionModalOpen} onClose={() => setDecisionModalOpen(false)} maxWidth="sm" fullWidth>
                    <DialogTitle>{t('recordDecision')}</DialogTitle>
                    <DialogContent>
                    ...
                    <DialogActions>
                        <Button onClick={() => setDecisionModalOpen(false)}>{tCommon('cancel')}</Button>
                ...
                <Dialog open={visibilityModalOpen} onClose={() => setVisibilityModalOpen(false)}>
                    ...
                    <DialogActions>
                        <Button onClick={() => setVisibilityModalOpen(false)}>{tCommon('cancel')}</Button>
                        <Button onClick={handleVisibilityUpdate} variant="contained" disabled={!newVisibility}>{tCommon('update') || 'Update'}</Button>
                    </DialogActions>
                </Dialog>
                ...
                <Dialog open={noteModalOpen} onClose={() => setNoteModalOpen(false)} fullWidth>
                    ...
                    <DialogActions>
                        <Button onClick={() => setNoteModalOpen(false)}>{tCommon('cancel')}</Button>
                        <Button onClick={handleAddNote} variant="contained" disabled={!newNote.trim()}>{t('addNote')}</Button>
                    </DialogActions>
                </Dialog>
