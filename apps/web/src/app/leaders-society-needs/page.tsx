    ...
    if (error) {
        return (
            <Container maxWidth={false} sx={{ py: 4, px: { xs: 2, sm: 3 } }}>
                <Alert severity="error">Failed to load leaders: {error.message}</Alert>
            </Container>
        );
    }
            ...
            <Container maxWidth={false} sx={{ px: { xs: 2, sm: 3 } }}>
                ...
                <Alert severity="info" sx={{ mb: 3 }}>
                    ...
                    <Typography variant="body2">
                        The profiles featured here represent contributor opinions and community observations. This is not an official endorsement, ranking, or political affiliation platform.
                    </Typography>
                </Alert>
                ...
                <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 4 }}>
                    <Box>
                        ...
                        <Typography variant="body1" color="text.secondary">
                            Documenting grassroots voices on community service and social contribution
                        </Typography>
                    </Box>
                    ...
                    {isSuperAdmin && (
                        <Button
                            variant="contained"
                            onClick={() => router.push('/submit-leader')}
                ...
                {leaders.length === 0 ? (
                    ...
                    <Box sx={{ display: 'flex', flexDirection: 'column', gap: 3 }}>
                        {leaders.map((leader) => (
                            <Card
                                key={leader.id}
                                onClick={() => router.push(`/leader-profile/${leader.id}`)}
                                sx={{
                                    display: 'flex',
                                        ...
                                        <Grid item xs={12} md={3} sx={{ display: 'flex', flexDirection: 'column', alignItems: { xs: 'flex-start', md: 'flex-end' }, justifyContent: 'center', gap: 1 }}>
                                            {isSuperAdmin && (
                                                <Button
                                                    ...
                                                    onClick={(e) => {
                                                        e.stopPropagation();
                                                        router.push(`/submit-leader?id=${leader.id}`);
                                                    }}
