...
function SubmitSocialWarriorContent() {
    ...
    const validate = () => {
        ...
        if (formData.digitalContributions.length === 0) {
            newErrors.digitalContributions = 'Select at least one digital contribution area';
        } else if (formData.digitalContributions.length > 5) {
            newErrors.digitalContributions = 'Select maximum 5 digital contribution areas';
        }

        if (formData.engagementStyle.length === 0) {
            newErrors.engagementStyle = 'Select at least one engagement style';
        } else if (formData.engagementStyle.length > 3) {
            newErrors.engagementStyle = 'Select maximum 3 engagement styles';
        }
            ...
            <Container maxWidth={false} sx={{ py: 8, px: { xs: 2, sm: 3 }, maxWidth: '900px', mx: 'auto' }}>
                ...
                <Paper sx={{ p: { xs: 3, sm: 4 }, boxShadow: '0 2px 8px rgba(0, 0, 0, 0.1)' }}>
                    <Typography variant="h5" sx={{ mb: 3, fontWeight: 600 }}>
                        {isEditMode ? 'Edit Social Media Warrior Profile' : 'Submit Social Media Warrior Profile'}
                    </Typography>
                    ...
                    <Box component="form" onSubmit={handleSubmit} noValidate sx={{ display: 'flex', flexDirection: 'column', gap: 2.5 }}>
                        ...
                        <TextField
                            label="Why This Person is a Valuable Digital Warrior"
                            value={formData.reason}
                            onChange={(e) => handleChange('reason', e.target.value)}
                            error={!!errors.reason}
                            helperText={errors.reason || `Describe specific digital contributions, campaigns, or online activism. ${formData.reason.length}/2000`}
                            fullWidth
                            required
                            multiline
                            rows={6}
                        ...
                        <FormControl error={!!errors.digitalContributions}>
                            <FormLabel>Digital Contribution Areas (Select 1-5)</FormLabel>
                            <FormGroup>
                        ...
                        <FormControl error={!!errors.engagementStyle}>
                            <FormLabel>Digital Engagement Style (Select 1-3)</FormLabel>
                            <FormGroup>
